...
module.exports = function expressify(router) {
  ...
  _.each(router.routes, function(uri, resourceId) {
    const resource = renderLinksOfResource(getResource(resourceId, router), router)
...
function renderLinksOfResource(resource, router) {
  return _.assign({}, resource, {links: renderLinks(resource.links, router)})
}

function renderLinks(links, router) {
  return _.mapKeys(links, function(link, resourceId) {
    return router[resourceId]
  })
}
